...
/**
 * Aug 13, 2012 3:41:41 PM
 *
 * This Zend class is used to read mo files
 * Array of translation is returned on success,
...
class Zend_Translate_Adapter_Gettext
{
     ...
     * @return array
     */
    public function loadTranslationData($filename, $locale, array $options = array())
    {
        $this->_data = array();
        $this->_bigEndian = false;
        $this->_file = @fopen($filename, 'rb');
        if (!$this->_file) {
            return "Error opening translation file '$filename'.";
        }
        if (@filesize($filename) < 10) {
            @fclose($this->_file);
            return "'$filename' is not a gettext file.";
        }
